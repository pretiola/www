...
6. Confirm the recipient, SMTP host/credentials and verified sender using secrets, not committed files. Set MAIL_MODE=smtp, SMTP_HOST, SMTP_USERNAME, SMTP_PASSWORD, MAIL_FROM and INTAKE_EMAIL. SMTP uses mandatory STARTTLS. Set PUBLIC_ORIGIN to the exact public origin. Verify the approved test message reaches the inbox; provider acceptance alone is insufficient.
...
Use `--db /data/pretiola/intake.sqlite3` before the subcommand in production. `show` produces private JSON suitable for restricted export; do not put its output in ordinary logs. Backups use SQLite's online backup API, not a raw file copy that omits WAL data. Restore with the application stopped to a new private directory, verify integrity, preserve 0600 permissions, and point a capture-only isolated instance at it. Never point a restored test instance at live SMTP. Reapply documented deletion requests and expire records older than 90 days before restored service resumes. Backup retention/deletion must be approved and enforced separately; deleting the live row does not erase existing snapshots.

Notifications contain only route and reference, not contact details or message contents. Local capture marks jobs `captured`, never `sent`. Production retries start after one minute with exponential backoff and stop after eight attempts; inspect failed jobs and requeue explicitly after repair. A crash between email acceptance and recording its result can produce a duplicate notification with the same Message-ID. No new inquiry is created by notification retry.
