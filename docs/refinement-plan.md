...
- No pushes, pull requests, production deployment, production infrastructure changes or real external test messages until explicitly authorized. Local commits are authorized. A push to main currently triggers deployment; release authorization must cover that consequence.
...
- [ ] Document reproducible startup, asset builds and test commands; preserve the existing passing baseline.
- [ ] Inspect the generated CSS difference from initial setup before including it in any commit.
- [ ] Correct robots.txt to the Pretiola sitemap; make `/` the canonical homepage and redirect `/index.html`.
- [ ] Use explicit public routes/sitemap entries so partials and draft templates cannot become public pages automatically.
- [ ] Triage the dependency audit findings from setup and apply compatible fixes where supported; separate any necessary breaking upgrade.
...
- [ ] Add a first-party submission endpoint with server-side validation, bounded inputs and typed contributor/ministry records.
- [ ] Decide and document durable storage and migrations before implementation. Evaluate a local SQLite database for development against the eventual Fly.io topology; do not assume an ephemeral container filesystem is durable or that one attached volume supports multiple machines.
- [ ] Save inquiry and notification job atomically, then confirm receipt. Return an error rather than success if saving fails.
- [ ] Add submission identifiers, duplicate protection and accessible abuse controls that require no JavaScript challenge.
- [ ] Provide restricted retrieval/export and a deletion process; a public admin dashboard is not required.
- [ ] Document backup and restore procedures and exercise them against synthetic records.
...
- [ ] Add a notification adapter and local capture sink; keep email delivery separate from the saved record.
- [ ] Record pending/sent/failed notification state, bounded retry behavior and actionable failures. Do not log inquiry bodies or credentials.
- [ ] Define the eventual sender/recipient configuration and delivery acceptance test, without enabling live delivery locally.
- [ ] Remove obsolete Web3Forms configuration after its final caller is replaced in stages 4–5.
...
- [ ] Move intake to `/for_ministries.html`, preserving a useful destination at the old homepage anchor.
- [ ] Require organization, contact name, reply email, country and practical need. Move financial/canonical/payment questions to optional discovery or remove them from first contact.
- [ ] Use native POST, meaningful labels, server-rendered errors with preserved input, and a real confirmation page after successful persistence.
- [ ] Keep submit usable for validation; avoid the CSS-hidden thank-you and honeypot behavior exposed in Lynx. Make email fallback available before failure.
- [ ] Remove the three-day reply promise unless its operational owner confirms it.
...
- [ ] Build `/contribute.html` with scoped professional work, mentoring/review and supervised learning clearly distinguished by real availability.
- [ ] Add a separate introduction form: name, reply email, mode and experience required; availability, time zone, interests and referral optional.
- [ ] Connect to the owned submission flow with route-specific validation and notification content.
- [ ] Explain commitment, support and how to pause. Until a real assignment is confirmed, use honest expressions of interest rather than an invented vacancy or guaranteed placement.
...
- [ ] Draft the contributor-first invitation and practical work categories from the guide.
- [ ] Order the page around invitation, practical work, participation process, attributable example, founder/faith and next action.
- [ ] Use consistent About, How we help, Our work, contributor and ministry destinations throughout shared navigation/footer.
- [ ] Preserve useful old anchors, repair the unfinished footer sentence, and expose contact/privacy/terms on every page.
...
- [ ] Refine the founder/faith section while distinguishing independent Catholic inspiration from official endorsement.
...
- [ ] Replace irrelevant financial/advertising boilerplate with accurate website/inquiry scope; flag legal wording requiring qualified review.
- [ ] Decide analytics behavior and keep local development out of production measurement. If retained, use consent behavior appropriate to the agreed setup and send no personal information.
...
- [ ] Define a restricted operational register for inquiry → conversation → match → commitment → accepted deliverable, with clear denominators and observation windows.
...
- [ ] Improve typography, spacing, section rhythm, contrast and responsive layout while retaining the existing character.
- [ ] Correct heading hierarchy, link names, focus visibility, anchor offsets and mobile navigation state/Escape behavior. Keep navigation available when JavaScript is off.
- [ ] Update page titles, descriptions, canonical and social metadata to the agreed positioning; use approved imagery only.
...
- [ ] Present the working local site, completed checklist, remaining factual/operational decisions and deployment/rollback requirements for explicit sign-off.
- [ ] Stop before pushing. User approval of the local changes does not silently authorize a production rollout unless that is included in the instruction.
...
| Stage | Status | Commit / verification / open feedback |
| --- | --- | --- |
| Plan | Prepared | Local-only workflow recorded; baseline has six passing Rust tests |
| 1–10 | Proposed | No implementation completed under this plan yet |
